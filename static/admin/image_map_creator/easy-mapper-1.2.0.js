...
function generateCode() {
	...
	for (i=0;i<cnt;i++) {
		...
		// 마크업 루프 생성
		gcAloop += '&lt;a xlink:href="../error_appearance/' + link + '"&gt;&lt;rect id="'+ link +'" class="routemodel_area" y="' + top  + '" x="' + left  + '" width="' + width + '" height="' + height + '" fill="#fff" opacity="0" /&gt;&lt;/a&gt;';
	...
	$('#final_image_map').html(gcA)
	// 이미지맵 마크업
	//var gcM = '&lt;map name="routemodel"&gt;' + '<br>' + gcMloop + '&lt;/map&gt;';
	//$('#pop-codegen-im .pop-content p').html(gcM); // 생성된 마크업 출력
	//$('#final_image_map').html(gcM)
}
...
function addMapElView(index) {
	$img.append('<div class="grid-box" id="grid-box-' + index + '"><span class="grid-box-cnt">' + (index + 1) + '</span><span class="grid-box-close">&times;</span><span class="grid-box-link">Link hinzufügen</span></div>');
...
function addLinkToMapEl() {
	...
	if ($('.grid-box._active').hasClass('_added')) {
		...
		$('#pop-addlink .pop-title').text('URL Link hinzufügen');
		$('#pop-addlink .pop-btn-confirm').text('Link hinzufügen');
...
function setLinkToMapEl() {
	...
	if ($('#pop-addlink-input option:selected').val()) {
		mapEl[addIndex][2] = $('#pop-addlink-input option:selected').val();
